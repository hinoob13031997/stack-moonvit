import {STACK_LIBRARY,TODAY} from './data.js?v=30';
...
class StackStore{
  ...
  actions(code=this.state.activeStack,date=TODAY()){const record=this.record(date),historical=date!==TODAY(),base=this.stack(code).actions.filter(action=>!action.checkin&&(this.templateScheduled(action.id,date)||historical&&record?.done.includes(action.id))&&(historical||this.state.moonConnected||!action.product));const custom=this.state.customActions.map(action=>this.actionAt(action,date)).filter(action=>action&&action.stackCode===code&&this.isScheduled(action,date)).sort((a,b)=>(a.order??0)-(b.order??0));return [...base,...custom]}
  ...
  isScheduled(action,date=TODAY()){
    ...
    if(date!==TODAY()&&hasHistory)return true;
