...
function workerHarness(){
  ...
  const caches={open:async()=>cache,keys:async()=>['stack-moonvit-shell-v30','stack-moonvit-shell-v31','stack-moonvit-shell-v32'],delete:async key=>{deleted.push(key);return true},match:async key=>cacheHits.get(typeof key==='string'?key:key.url)};
...
test('service worker устанавливает v33 и удаляет только старые кеши',async()=>{
  const worker=workerHarness();
  await runWait(worker.listeners.install);
  assert.equal(worker.state().skipped,true);
  assert.ok(worker.added.includes('./src/app.js?v=33'));
  await runWait(worker.listeners.activate);
  assert.deepEqual(worker.deleted,['stack-moonvit-shell-v30','stack-moonvit-shell-v31','stack-moonvit-shell-v32']);
...
test('ресурсы берутся из кеша, а чужой origin не перехватывается',async()=>{
  ...
  const asset={method:'GET',mode:'cors',url:'https://example.test/src/app.js?v=33'};
