...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/styles\/app\.css\?v=41/);
  assert.match(index,/src\/app\.js\?v=41/);
  assert.match(app,/sw\.js\?v=41/);
  assert.doesNotMatch(app,/data-planet-pulse/);
  assert.match(ui,/APP_VERSION='41'/);
  assert.match(ui,/class="planet-art"/);
  assert.doesNotMatch(ui,/data-planet-pulse/);
  assert.match(ui,/assets\/planets\/\$\{code\.toLowerCase\(\)\}\.webp\?v=41/);
  ...
  assert.match(store,/data\.js\?v=41/);
  assert.match(store,/planetProgress/);
  assert.match(worker,/stack-moonvit-shell-v41/);
  assert.match(worker,/assets\/planets\/capital\.webp\?v=41/);
