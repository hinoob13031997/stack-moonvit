...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/manifest\.webmanifest\?v=35/);
  assert.match(index,/styles\/app\.css\?v=35/);
  assert.match(index,/src\/app\.js\?v=35/);
  assert.match(app,/sw\.js\?v=35/);
  assert.match(ui,/APP_VERSION='35'/);
  assert.match(store,/data\.js\?v=35/);
  assert.match(worker,/stack-moonvit-shell-v35/);
