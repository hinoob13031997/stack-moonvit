...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/manifest\.webmanifest\?v=31/);
  assert.match(index,/styles\/app\.css\?v=31/);
  assert.match(index,/src\/app\.js\?v=31/);
  assert.match(app,/sw\.js\?v=31/);
  assert.match(ui,/APP_VERSION='31'/);
  assert.match(store,/data\.js\?v=31/);
  assert.match(worker,/stack-moonvit-shell-v31/);
