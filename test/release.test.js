...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/manifest\.webmanifest\?v=32/);
  assert.match(index,/styles\/app\.css\?v=32/);
  assert.match(index,/src\/app\.js\?v=32/);
  assert.match(app,/sw\.js\?v=32/);
  assert.match(ui,/APP_VERSION='32'/);
  assert.match(store,/data\.js\?v=32/);
  assert.match(worker,/stack-moonvit-shell-v32/);
