...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/styles\/app\.css\?v=38/);
  assert.match(index,/src\/app\.js\?v=38/);
  assert.match(app,/sw\.js\?v=38/);
  assert.match(ui,/APP_VERSION='38'/);
  ...
  assert.match(store,/data\.js\?v=38/);
  assert.match(store,/planetProgress/);
  assert.match(worker,/stack-moonvit-shell-v38/);
