...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/styles\/app\.css\?v=34/);
  assert.match(index,/src\/app\.js\?v=34/);
  assert.match(app,/sw\.js\?v=34/);
  assert.match(ui,/APP_VERSION='34'/);
  assert.match(store,/data\.js\?v=34/);
  assert.match(worker,/stack-moonvit-shell-v34/);
