...
test('версия интерфейса, ресурсов и автономного кеша согласована',async()=>{
  ...
  assert.match(index,/manifest\.webmanifest\?v=39/);
  assert.match(index,/styles\/app\.css\?v=39/);
  assert.match(index,/src\/app\.js\?v=39/);
  assert.match(app,/sw\.js\?v=39/);
  assert.match(ui,/APP_VERSION='39'/);
  ...
  assert.match(store,/data\.js\?v=39/);
  assert.match(store,/planetProgress/);
  assert.match(worker,/stack-moonvit-shell-v39/);
